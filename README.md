...
4. **用静态目标校几何和弹道。** 写入实测 `bullet_speed`，在近、中、远距离验证。固定 yaw 误差才
   使用 `yaw_offset`；pitch 在 `pitch_offset.near/far` 指定的两个水平距离分别标定，区间内线性插值、
   区间外保持端点值。若误差趋势无法由两点补偿解释，优先检查内参、外参、单位和弹速。
