...
rm_video::CompressionFrame make_frame(std::uint8_t seed)
{
  ...
  frame.processed = rm_video::BgrImage{2, 2, std::vector<std::uint8_t>(12, seed + 1)};
  return frame;
}
...
int main(int argc, char ** argv)
{
  ...
  auto publisher_node = std::make_shared<rclcpp::Node>("compressed_image_publisher_test_source");
  auto subscriber_node = std::make_shared<rclcpp::Node>("compressed_image_publisher_test_sink");
  ...
  check(!demand.raw && !demand.processed, "image outputs should be idle without subscribers");
  ...
  auto processed_subscription =
    subscriber_node->create_subscription<sensor_msgs::msg::CompressedImage>(
      ...
      [&processed_messages](sensor_msgs::msg::CompressedImage::ConstSharedPtr message) {
  ...
  check(
    wait_for([&processed_messages]() { return processed_messages.load() >= 1; }),
    "processed JPEG should be published to its subscriber");

  std::atomic<int> raw_messages{0};
  auto raw_subscription = subscriber_node->create_subscription<sensor_msgs::msg::CompressedImage>(
    "/test/image_raw", qos,
    [&raw_messages](sensor_msgs::msg::CompressedImage::ConstSharedPtr message) {
      if (!message->data.empty()) {
  ...
  check(
    wait_for([&raw_messages, &processed_messages]() {
