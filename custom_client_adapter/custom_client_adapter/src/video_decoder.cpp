...
bool parse_header(const std::uint8_t *data, std::size_t size,
                  std::size_t max_frame_bytes, PacketHeader &output) {
  ...
  const PacketHeader network_order{ntohs(frame_sequence),
                                   ntohs(fragment_sequence), ntohl(total_size)};
...
VideoDecoder::VideoDecoder(DecoderConfig config, FrameCallback callback,
                           rclcpp::Logger logger)
    : config_(std::move(config)), callback_(std::move(callback)),
      logger_(std::move(logger)), assemblies_(config_.reassembly_slots) {}
...
bool VideoDecoder::start() {
  ...
  RCLCPP_INFO(logger_, "Listening for %s video on %s:%d", codec_->name,
...
Statistics VideoDecoder::take_statistics() {
  return Statistics{decoded_frames_window_.exchange(0),
                    udp_packets_window_.exchange(0), dropped_.load(),
                    error_.load()};
}
...
bool VideoDecoder::initialize_decoder() {
  ...
  if (open_result < 0) {
    RCLCPP_ERROR(logger_, "Failed to open FFmpeg decoder: %s",
...
bool VideoDecoder::setup_socket() {
  ...
  if (socket_fd_ < 0) {
    RCLCPP_ERROR(logger_, "Failed to create UDP socket: %s",
                 std::strerror(errno));
    return false;
  }

  const int reuse_address = 1;
  setsockopt(socket_fd_, SOL_SOCKET, SO_REUSEADDR, &reuse_address,
             sizeof(reuse_address));
  if (setsockopt(socket_fd_, SOL_SOCKET, SO_RCVBUF,
                 &config_.receive_buffer_bytes,
                 sizeof(config_.receive_buffer_bytes)) < 0) {
    RCLCPP_WARN(logger_, "Failed to set UDP receive buffer: %s",
                std::strerror(errno));
  }

  const timeval timeout{config_.receive_timeout_ms / 1000,
                        (config_.receive_timeout_ms % 1000) * 1000};
  if (setsockopt(socket_fd_, SOL_SOCKET, SO_RCVTIMEO, &timeout,
                 sizeof(timeout)) < 0) {
    RCLCPP_ERROR(logger_, "Failed to set UDP receive timeout: %s",
                 std::strerror(errno));
    return false;
  }
  ...
  if (inet_pton(AF_INET, config_.bind_address.c_str(), &address.sin_addr) !=
      1) {
    RCLCPP_ERROR(logger_, "Invalid IPv4 bind address: %s",
                 config_.bind_address.c_str());
    return false;
  }
  if (bind(socket_fd_, reinterpret_cast<sockaddr *>(&address),
           sizeof(address)) < 0) {
    RCLCPP_ERROR(logger_, "Failed to bind UDP socket: %s",
                 std::strerror(errno));
    return false;
  }
  return true;
...
void VideoDecoder::process_datagram(const std::uint8_t *data,
                                    std::size_t size) {
  expire_assemblies(std::chrono::steady_clock::now());
  ...
  if (payload_size == 0 || payload_size > header.total_size) {
    dropped_.fetch_add(1);
    RCLCPP_WARN(logger_,
                "Dropped UDP packet from frame %u: invalid payload size",
                header.frame_sequence);
    return;
  }

  FrameAssembly &assembly =
      get_or_create_assembly(header.frame_sequence, header.total_size);
  if (assembly.fragments.find(header.fragment_sequence) !=
      assembly.fragments.end()) {
    dropped_.fetch_add(1);
    RCLCPP_WARN(logger_,
                "Dropped UDP packet from frame %u: duplicate fragment %u",
                header.frame_sequence, header.fragment_sequence);
    return;
  }

  assembly.fragments.emplace(
      header.fragment_sequence,
      std::vector<std::uint8_t>(data + kPacketHeaderSize, data + size));
  ...
  complete_frame.data.reserve(assembly.total_size +
                              AV_INPUT_BUFFER_PADDING_SIZE);
  complete_frame.packet_count = assembly.fragments.size();
  for (const auto &[sequence, fragment] : assembly.fragments) {
    (void)sequence;
    complete_frame.data.insert(complete_frame.data.end(), fragment.begin(),
                               fragment.end());
  }
...
VideoDecoder::FrameAssembly &
VideoDecoder::get_or_create_assembly(std::uint16_t frame_sequence,
                                     std::uint32_t total_size) {
...
void VideoDecoder::expire_assemblies(
    std::chrono::steady_clock::time_point now) {
  for (auto &assembly : assemblies_) {
    if (assembly.active &&
        now - assembly.last_update > config_.reassembly_timeout) {
...
void VideoDecoder::submit_frame(PendingFrame frame) {
  {
    ...
    if (pending_frames_.size() >= config_.decode_queue_size) {
      ...
      const std::uint16_t first_sequence =
          pending_frames_.front().frame_sequence;
      ...
      RCLCPP_WARN(logger_,
                  ...
                  pending_frames_.size(), first_sequence, last_sequence,
                  packet_count, frame.frame_sequence);
      pending_frames_.clear();
...
void VideoDecoder::decode_frame(PendingFrame pending) {
  ...
  while (true) {
    ...
    if ((frame_->flags & AV_FRAME_FLAG_CORRUPT) != 0 ||
        frame_->decode_error_flags != 0 || !submit_output_frame(*frame_)) {
      packet_failed = true;
  ...
  while (true) {
    AvFramePtr source;
    {
      ...
      output_condition_.wait(lock, [this]() {
        return !running_.load() || pending_output_frame_ != nullptr;
      });
      if (!running_.load()) {
        return;
      }
...
void VideoDecoder::discard_assembly(FrameAssembly &assembly,
                                    const char *reason) {
  const std::size_t packet_count = assembly.fragments.size();
  dropped_.fetch_add(packet_count);
  if (packet_count > 0) {
    stream_discontinuity_.store(true);
    RCLCPP_WARN(logger_, "Dropped incomplete frame %u (%zu UDP packets): %s",
                assembly.frame_sequence, packet_count, reason);
  }
  assembly.reset();
}
